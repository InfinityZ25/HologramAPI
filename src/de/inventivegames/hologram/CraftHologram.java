...
abstract class CraftHologram implements Hologram {
	...
	protected void buildPackets(boolean rebuild) throws Exception {
		...
		if (HologramAPI.is1_8 && !HologramAPI.useProtocolSupport) {
			Object armorStand = ClassBuilder.buildEntityArmorStand(world,
					this.getLocation().add(0, HologramOffsets.ARMOR_STAND_DEFAULT, 0), this.getText());
			...
			this.teleportPacketArmorStand = ClassBuilder.buildTeleportPacket(this.hologramIDs[0],
					this.getLocation().add(0, HologramOffsets.ARMOR_STAND_DEFAULT, 0), true, false);
		} else {

			// Spawn Horse
			Object horse_1_7 = ClassBuilder.buildEntityHorse_1_7(world,
					this.getLocation().add(0, HologramOffsets.WITHER_SKULL_HORSE, 0), this.getText());
			Object horse_1_8 = ClassBuilder.buildEntityHorse_1_8(world,
					this.getLocation().add(0, HologramOffsets.ARMOR_STAND_PACKET, 0), this.getText());

			// Spawn WitherSkull
			Object witherSkull_1_7 = ClassBuilder.buildEntityWitherSkull(world,
					this.getLocation().add(0, HologramOffsets.WITHER_SKULL_HORSE, 0));
			...
			if (Minecraft.VERSION.olderThan(Minecraft.Version.v1_9_R1)) {
				...
				AccessUtil.setAccessible(NMSClass.PacketPlayOutAttachEntity.getDeclaredField("b"))
						.set(this.attachPacket, this.hologramIDs[1]);
				AccessUtil.setAccessible(NMSClass.PacketPlayOutAttachEntity.getDeclaredField("c"))
						.set(this.attachPacket, this.hologramIDs[0]);
			} else {
			...
			if (!HologramAPI.is1_8 || HologramAPI.useProtocolSupport) {
				this.teleportPacketSkull = ClassBuilder.buildTeleportPacket(this.hologramIDs[0],
						this.getLocation().add(0, HologramOffsets.WITHER_SKULL_HORSE, 0), true, false);
			...
			this.teleportPacketHorse_1_8 = ClassBuilder.buildTeleportPacket(this.hologramIDs[2],
					this.getLocation().add(0, HologramOffsets.ARMOR_STAND_PACKET, 0), true, false);
		}

		/* Touch packets */
		if (this.isTouchable()) {
			int size = this.getText() == null ? 1 : (int) (this.getText().length() / 2 / 3);
			Object touchSlime = ClassBuilder.buildEntitySlime(world,
					this.getLocation().add(0, HologramOffsets.TOUCH_SLIME_SKULL, 0), size);
			...
			if (HologramAPI.is1_8 && !HologramAPI.useProtocolSupport) {
				touchVehicle = ClassBuilder.buildEntityArmorStand(world, this.getLocation().add(0,
						HologramOffsets.ARMOR_STAND_PACKET + HologramOffsets.TOUCH_SLIME_ARMOR_STAND, 0), null);
				...
				touchVehicle = ClassBuilder.buildEntityWitherSkull(world,
						this.getLocation().add(0, HologramOffsets.TOUCH_SLIME_SKULL, 0));
				this.dataWatcherTouchVehicle = AccessUtil.setAccessible(NMSClass.Entity.getDeclaredField("datawatcher"))
						.get(touchVehicle);
			...
			if (Minecraft.VERSION.olderThan(Minecraft.Version.v1_9_R1)) {
				...
				AccessUtil.setAccessible(NMSClass.PacketPlayOutAttachEntity.getDeclaredField("b"))
						.set(this.attachPacketTouch, this.touchIDs[0]);
				AccessUtil.setAccessible(NMSClass.PacketPlayOutAttachEntity.getDeclaredField("c"))
						.set(this.attachPacketTouch, this.touchIDs[1]);
			} else {
			...
			this.teleportPacketTouchSlime = ClassBuilder.buildTeleportPacket(this.touchIDs[0],
					this.getLocation().add(0, HologramOffsets.TOUCH_SLIME_SKULL, 0), true, false);
			if (HologramAPI.is1_8 && !HologramAPI.useProtocolSupport) {
				this.teleportPacketTouchVehicle = ClassBuilder.buildTeleportPacket(this.touchIDs[1],
						this.getLocation().add(0,
								HologramOffsets.ARMOR_STAND_PACKET + HologramOffsets.TOUCH_SLIME_ARMOR_STAND, 0),
						true, false);
			} else {
				this.teleportPacketTouchVehicle = ClassBuilder.buildTeleportPacket(this.touchIDs[1],
						this.getLocation().add(0, HologramOffsets.TOUCH_SLIME_SKULL, 0), true, false);
		...
		if (Minecraft.VERSION.olderThan(Minecraft.Version.v1_9_R1)) {
			...
			AccessUtil.setAccessible(NMSClass.PacketPlayOutAttachEntity.getDeclaredField("a"))
					.set(this.ridingAttachPacket, 0);
			AccessUtil.setAccessible(NMSClass.PacketPlayOutAttachEntity.getDeclaredField("a"))
					.set(this.ridingEjectPacket, 0);

			AccessUtil.setAccessible(NMSClass.PacketPlayOutAttachEntity.getDeclaredField("b"))
					.set(this.ridingAttachPacket, this.hologramIDs[0]);
			AccessUtil.setAccessible(NMSClass.PacketPlayOutAttachEntity.getDeclaredField("b"))
					.set(this.ridingEjectPacket, this.hologramIDs[0]);

			AccessUtil.setAccessible(NMSClass.PacketPlayOutAttachEntity.getDeclaredField("c")).set(
					this.ridingAttachPacket, this.getAttachedTo() != null ? this.getAttachedTo().getEntityId() : -1);
			AccessUtil.setAccessible(NMSClass.PacketPlayOutAttachEntity.getDeclaredField("c"))
					.set(this.ridingEjectPacket, -1);
		} else {
			...
			AccessUtil.setAccessible(NMSClass.PacketPlayOutMount.getDeclaredField("a")).set(this.ridingAttachPacket,
					((DefaultHologram) this).isAttached() && this.getAttachedTo() != null
							? this.getAttachedTo().getEntityId()
							: -1);
	...
	protected void sendNamePackets(final Collection<? extends Player> receivers) {
		for (Player p : receivers) {
			try {
				int id = HologramAPI.is1_8 && !HologramAPI.useProtocolSupport ? this.hologramIDs[0]
						: HologramAPI.getVersion(p) > 5 ? this.hologramIDs[2] : this.hologramIDs[1];
				Object dataWatcher = HologramAPI.is1_8 && !HologramAPI.useProtocolSupport ? this.dataWatcherArmorStand
						: HologramAPI.getVersion(p) > 5 ? this.dataWatcherHorse_1_8 : this.dataWatcherHorse_1_7;
				...
				if (HologramAPI.getVersion(p) <= 5) {
					if (this.hologramIDs.length > 1) {
						HologramAPI.sendPacket(p, ClassBuilder.buildNameMetadataPacket(this.hologramIDs[1],
								this.dataWatcherHorse_1_7, 10, 11, this.getText()));
					}
				}
			} catch (Exception e) {
				e.printStackTrace();
	...
	public int hashCode() {
		...
		result = prime * result
				+ (this.teleportPacketArmorStand == null ? 0 : this.teleportPacketArmorStand.hashCode());
		...
		result = prime * result
				+ (this.teleportPacketTouchSlime == null ? 0 : this.teleportPacketTouchSlime.hashCode());
		result = prime * result
	...
	public boolean equals(Object obj) {
		...
		if (!Arrays.equals(this.touchIDs, other.touchIDs)) {
			return false;
		}
		return true;
	}
